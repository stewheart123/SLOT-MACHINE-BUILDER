/**
 * creates the elements used in the main game such as buttons and containers
 * it loads assets from the asset manifest
 * need to re-introduce the redraw function to allow responsive screens
 */
class SceneLoader {
  constructor() {
    ...
    this.gameUIPanel = new PIXI.Graphics();
  }
  ...
  async makeGameScreen() {
    ...
    const backgroundImage = new PIXI.Sprite(
      this.gameScreenAssets.backgroundImage
    );
    ...
    app.ticker.add(() => {
      if (!gameContainerAnimationComplete) {
        const distance = Math.sqrt(
          Math.pow(
            this.gameContainer.position.x - gameContainerEndPosition.x,
            2
          ) +
            Math.pow(
              this.gameContainer.position.y - gameContainerEndPosition.y,
              2
            )
        );
    ...
    this.gameUIPanel.beginFill(0x000000, 0.5);
    this.gameUIPanel.lineStyle(2, 0xffffff, 4);
    this.gameUIPanel.drawRect(0, 0, gameUIPanelWidth, footerHeight);
    this.gameUIPanel.endFill();
    this.gameUIContainer.addChild(this.gameUIPanel);
    app.stage.addChild(this.gameUIContainer);

    this.spinButton.beginFill(0x000000, 0.5);
    this.spinButton.lineStyle(2, 0xffffff, 4);
    this.spinButton.drawCircle(this.gameUIPanel.width - 50, 50, 50);
    this.spinButton.endFill();
    this.gameUIPanel.addChild(this.spinButton);
    ...
    window.addEventListener("resize", this.plotGraphics.bind(this));
  ...
  plotGraphics() {
    ...
    //   this.spinButton.beginFill(0x000000, 0.5);
    //   this.spinButton.lineStyle(2, 0xffffff, 4);
    //   this.spinButton.drawCircle(this.gameUIPanel.width - 54, 50, 50);
    //   this.spinButton.endFill();
